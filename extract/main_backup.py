...
def process_repo(repo_path: Path, repo_name: str, filter_large_functions: bool = False) -> dict:
    ...
    for file_path in repo_path.rglob("*"):
        ...
        # 收集函数?        for func in result["functions"]:
    ...
    for file_path in repo_path.rglob("*"):
        ...
        # 只打印函数信息，不打印类信息
        if result["functions"]:
            print(f"  ?{file_path.relative_to(repo_path)} 中找?{len(result['functions'])} 个函?)
    
    total_functions_before_filter = len(all_functions)
    
    # 如果启用大函数过滤，则过滤函?    if filter_large_functions:
        filtered_functions = []
        ...
        print(f"\n 启用大函数过滤：?{total_functions_before_filter} 个函数中筛选出 {len(all_functions)} 个代码行?{LARGE_FUNCTION_THRESHOLD}的函?)
    
    print(f"\n 完成分析 {file_count} 个文件，共找?{len(all_functions)} 个函?)

    # 提取所有函数名
    function_names = [func["basic_info"]["function_name"] for func in all_functions]

    # 构建API调用关系?    call_graph = build_api_call_graph(all_functions, repo_path)
...
def print_analysis_summary(result: dict) -> None:
    ...
    if api_relationships:
        ...
        print(f"  - 孤立函数? {call_graph.get('statistics', {}).get('isolated_functions', 0)}")
        ...
        if most_called:
            ...
            for i, func in enumerate(most_called[:5]):
                print(f"    {i+1}. {func['function_name']} (被调?{func['call_count']} ?")
        ...
        if any(circular_deps.values()):
            ...
            for level, cycles in circular_deps.items():
                if cycles:
                    print(f"    {level}: {len(cycles)} 个循?)
        
        # 显示最复杂的函?        most_complex = metrics.get('overall_metrics', {}).get('most_complex_functions', [])
        if most_complex:
            print(f"  - 最复杂的函?")
            for i, func in enumerate(most_complex[:5]):
                print(f"    {i+1}. {func['function_name']} (复杂? {func['complexity']})")
    
    # 显示传递调用关系分析结?    transitive_calls_result = result.get("transitive_calls", {})
    if transitive_calls_result:
        ...
        print(f"\n 传递调用关系分析结?")
        print(f"  - 传递调用图节点? {transitive_graph.get('statistics', {}).get('total_nodes', 0)}")
        print(f"  - 传递调用边? {len(transitive_graph.get('edges', []))}")
        print(f"  - 最大传递调用深? {transitive_graph.get('statistics', {}).get('max_transitive_depth', 0)}")
        print(f"  - 有传递调用的函数? {transitive_graph.get('statistics', {}).get('functions_with_transitive_calls', 0)}")

        print(f"  - 传递调用最多的函数:")
        for i, func in enumerate(impact_analysis.get('most_transitive_functions', [])[:5]):
            print(f"    {i+1}. {func['function_name']} (传递调?{func['transitive_call_count']} ?")

        print(f"  - 最长传递调用链 (?0):")
        for i, chain in enumerate(impact_analysis.get('transitive_call_chains', [])[:10]):
            print(f"    {i+1}. {chain['description']} (长度: {chain['length']})")

        print(f"  - 高影响函?(被很多函数间接调用的函数):")
        for i, func in enumerate(impact_analysis.get('high_impact_functions', [])[:10]):
            print(f"    {i+1}. {func['function_name']} (影响函数? {func['impact_count']})")
        ...
        for i, func in enumerate(impact_analysis.get('hub_functions', [])[:10]):
            print(f"    {i+1}. {func['function_name']} (总影? {func['total_influence']})")

        print(f"  - 叶子函数 (只被调用，不调用其他函数的函?:")
        for i, func_name in enumerate(impact_analysis.get('leaf_functions', [])[:20]):
            print(f"    {i+1}. {func_name}")
        
        # 显示重要度分析结?        importance_scores = impact_analysis.get('importance_scores', {})
        if importance_scores:
            print(f"\n  === 函数重要度分析结?===")
            
            # 显示重要度分?            distribution = importance_scores.get('importance_distribution', {})
            print(f"  - 重要度分?")
            for level, count in distribution.items():
                print(f"    {level}: {count} 个函?)
            
            # 显示?0名重要函?            top_functions = importance_scores.get('top_functions', [])
            if top_functions:
                print(f"  - ?0名重要函?")
                for i, func in enumerate(top_functions[:10]):
                    print(f"    {i+1}. {func['function_name']} (重要? {func['total_score']}, 等级: {func['importance_level']})")
            
            # 显示Critical级别函数的详细信?            critical_functions = []
            ...
            if critical_functions:
                ...
                for i, (func_name, score_info) in enumerate(critical_functions[:5]):
                    ...
                    if 'direct_call_weight' in breakdown:
                        ...
                        print(f"      复杂度权? {breakdown.get('complexity_weight', 'N/A')}")
                    ...
                    if 'is_coordinator' in metrics:
                        print(f"      是否协调? {metrics['is_coordinator']}")
...
def main():
    ...
    # 解析命令行参?    filter_large_functions = "--filter-large-functions" in sys.argv
    # 已移除Neo4j直接写入功能，只保留JSON文件生成
    
    # 检查是否有稀疏检出路?    sparse_checkout_paths = None
    for arg in sys.argv[2:]:
        if arg not in ["--filter-large-functions"]:
            # 分割多个路径（用逗号分隔?            sparse_checkout_paths = [path.strip() for path in arg.split(',')]
            break
    
    if filter_large_functions:
        print("启用大函数过滤模式：只保留代码行?200的函?)
    
    if sparse_checkout_paths:
        print(f"启用稀疏检出模式：只检出路?{', '.join(sparse_checkout_paths)}")
    ...
    # 保存结果为JSON文件
    output_dir = "output/extract_output"
    ...
    if len(function_names) > 10:
        print(f" ... 还有 {len(function_names) - 10} 个函?)
    
    print(f"\n  分析包含以下信息:")
    print(f"  (1) 基础信息: 原代码、代码定义位置、注释、函数名")
    print(f"  (2) 复杂? 语义复杂?lizard)、语法复杂度(Tree-sitter)、结构复杂度")
    print(f"  (3) 上下文信? 所在类/模块、import列表、函数调用、返回值使?)
    print(f"  (4) API调用关系: 调用图、依赖关系、循环依赖检测、API指标分析")
    print(f"  (5) 传递调用关? 传递调用图、影响范围分析、循环依赖检?)
    print(f"  (6) 函数重要? 多维度重要度计算、重要度等级分类")
