...
def extract_imports(file_path: str, lang: str, repo_path: Path) -> List[str]:
    ...
    """
    try:
        # 构建完整的文件路?        full_path = repo_path / file_path
        if not full_path.exists():
            print(f"文件不存在 {full_path}")
            return []
        
        # 获取语言解分分析        parser = get_parser(lang)
        if not parser:
            return []
        ...
        def collect_imports(node: Node, depth: int = 0):
            ...
            # Ruby导入语句
                ...
                if node.type == "call":
                    # 处理 require 'xxx' ?require_relative 'xxx'
            ...
            # 递归遍历子节?            for child in node.children:
...
def _should_filter_js_function(function_name: str) -> bool:
    """
    判断是否应该过滤掉某个JavaScript函数?    过滤掉不应该被提取的函数，如window对象属性赋值等
    """
    if not function_name:
        return True
    
    # 过滤掉window对象属性赋?    if function_name.startswith("window[") or function_name.startswith("window."):
        return True
    
    # 过滤掉其他全局对象属性赋?    global_objects = ["document", "console", "navigator", "location", "history", "screen", "global", "self"]
    for obj in global_objects:
        if function_name.startswith(f"{obj}[") or function_name.startswith(f"{obj}."):
            return True
    
    # 过滤掉包含方括号的属性访问（通常是对象属性赋值）
    # 但允许简单的数组访问，如 arr[0]
    if "[" in function_name and "]" in function_name:
        # 检查是否是简单的数组访问模式
        if not (function_name.count("[") == 1 and function_name.count("]") == 1):
            return True
        # 检查方括号内是否是数字或简单变?        bracket_start = function_name.find("[")
        bracket_end = function_name.find("]")
        if bracket_start != -1 and bracket_end != -1:
            inside_brackets = function_name[bracket_start+1:bracket_end].strip()
            # 如果方括号内包含引号，说明是对象属性访问，应该过滤
            if '"' in inside_brackets or "'" in inside_brackets:
                return True
    
    # 过滤掉包含点号的深层属性访问（如obj.prop.subprop?    if function_name.count(".") > 1:
        return True
    
    # 过滤掉以$开头的jQuery相关函数（通常是全局对象属性）
    if function_name.startswith("$") and len(function_name) > 1:
        return True
    
    # 过滤掉包含特殊字符的函数名（通常是动态属性访问）
...
def extract_js_function_name(node: Node, code_bytes: bytes) -> str:
    """
    专门用于提取JavaScript函数名的函数
    处理各种JavaScript函数定义方式
    """
    function_name = ""
    ...
    if node.type == "function_declaration":
        ...
        # tree-sitter JavaScript解分析器使?function"作为节点类型
        # 需要根据父节点类型来判断函数名
        if node.parent:
    ...
    if not function_name and node.parent:
        ...
        if node.type == "function_declaration" and node.parent.type == "program":
            # 这可能是全局构造函数            name_node = node.child_by_field_name("name")
            if name_node:
                function_name = get_node_text(name_node, code_bytes)
        ...
        # 检查是否是原型方法: MyClass.prototype.method = function() {}
            ...
            if left_node and left_node.type == "member_expression":
                # 提取原型方法?                property_node = left_node.child_by_field_name("property")
                if property_node:
...
def extract_function_info(node: Node, code_bytes: bytes, lang: str, file_path: str, repo_path: Path, all_function_names: Optional[Set[str]] = None) -> Dict:
    ...
    if lang == "python":
        ...
        # 使用专门的JavaScript函数名提取函数        function_name = extract_js_function_name(node, code_bytes)
        if function_name:
        ...
        # C/C++语言函数名通常在declarator?        declarator = node.child_by_field_name("declarator")
        if declarator:
    ...
    if name_node:
        # 检查是否是我们的DummyNode（通过检查是否有type属性来区分）
    ...
    # 获取源代?    source_code = get_node_text(node, code_bytes)
    
    # 分分析复杂?    complexity = analyze_complexity_metrics(node, code_bytes, source_code, lang)
...
def extract_class_info(node: Node, code_bytes: bytes, lang: str, file_path: str, all_function_names: Optional[Set[str]] = None) -> Dict:
    ...
    if lang == "python":
        ...
        # C++基类在base_clause?        base_clause = node.child_by_field_name("base_clause")
        if base_clause:
    ...
    if function_types:
        ...
        for child in node.children:
            if child.type in function_types:
                ...
                # 这里假设file_path是相对于repo_path的路?                repo_path = Path("target_project")
