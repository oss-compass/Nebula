...
def clone_repo(repo_url: str, sparse_checkout_paths: Optional[List[str]] = None, 
    ...
    Args:
        repo_url: 仓库URL
        sparse_checkout_paths: 稀疏检出路径列?        max_retries: 最大重试次?        use_ssh: 是否使用SSH协议（需要配置SSH密钥?        
    Returns:
        tuple: (本地路径, 仓库名称)
    """
    ...
    if not local_path.exists():
        ...
        git_configs = [
            # 标准配置
            [],
            # 禁用SSL验证（不推荐，但可以解决连接问题?            ["-c", "http.sslVerify=false"],
            # 使用较旧的SSL协议
            ...
            for config_idx, git_config in enumerate(git_configs):
                try:
                    ...
                    if sparse_checkout_paths:
                        # 稀疏检出模?                        logger.info(f"启用稀疏检出，只检出路? {', '.join(sparse_checkout_paths)}")
                        
                        # 初始化仓?                        cmd = ["git"] + git_config + ["clone", "--no-checkout", repo_url, str(local_path)]
        ...
        if not success:
            error_msg = f"?克隆仓库失败，已尝试 {max_retries} ? {repo_url}"
            if last_error:
                error_msg += f"\n最后错? {last_error}"
            logger.error(error_msg)
            raise Exception(error_msg)
    else:
        logger.info(f"仓库已存? {local_path}")
...
def extract_comments(node: Node, code_bytes: bytes, lang: str) -> List[Dict]:
    ...
    # 收集函数内部的注?    collect_comments(node)
