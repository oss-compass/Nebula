...
<<<<<<< HEAD
        ...
        for func in functions_list:
            # 检查是否已有复杂度信息且不需要重新计算
            if not recalculate_metrics and 'complexity' in func and func['complexity'] and func['complexity'].get('status') != 'pending':
                ...
                print(f"使用extract模块的复杂度信息: {func.get('basic_info', {}).get('function_name', 'unknown')}")
            else:
                # 重新计算复杂度（需要明确指定）
                complexity_info = calculate_complexity_score(func, None)
