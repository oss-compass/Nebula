...
class ProgressTracker:
    ...
    def start_repo(self, repo_name: str):
        ...
        with self.lock:
            ...
            print(f"\n{'='*80}")
            print(f"[{self.current_repo}/{self.total_repos}] 开始处理仓�? {repo_name}")
            print(f"{'='*80}")
...
class BatchProcessor:
    ...
    def __init__(self, 
        ...
        Args:
            txt_file: 包含GitHub库链接的txt文件路径
            neo4j_uri: Neo4j数据库URI
            neo4j_user: Neo4j用户�?            neo4j_password: Neo4j密码
            neo4j_database: Neo4j数据库名
            clean_db: 是否清空数据�?            filter_large_functions: 是否过滤大函�?            concurrent: 并发�?            batch_size: 批处理大�?            use_cache: 是否使用缓存
            model_name: 嵌入模型名称
            model_type: 嵌入模型类型
        """
    ...
    def read_repo_urls(self) -> List[str]:
        """读取GitHub库链接并添加.git后缀"""
        try:
            with open(self.txt_file, 'r', encoding='utf-8') as f:
                urls = [line.strip() for line in f if line.strip() and not line.startswith('#')]
            
            # 为每个链接添�?git后缀（如果还没有的话�?            processed_urls = []
            for url in urls:
                if url.endswith('.git'):
            ...
            logger.info(f"�?{self.txt_file} 读取�?{len(processed_urls)} 个GitHub库链�?)
            logger.debug(f"处理后的链接示例: {processed_urls[:3] if processed_urls else []}")
            return processed_urls
        except FileNotFoundError:
            logger.error(f"文件不存�? {self.txt_file}")
            raise
    ...
    def extract_repo_name(self, url: str) -> str:
        """从GitHub URL提取仓库名，与extract.main保持一�?""
        from urllib.parse import urlparse
        from pathlib import Path
        
        # 使用与extract.main相同的逻辑
        parsed = urlparse(url)
        repo_name = Path(parsed.path).stem  # 只取路径的最后一部分，去�?git
    ...
    def run_extract(self, repo_url: str) -> Optional[str]:
        ...
        try:
            ...
            # 执行命令并显示进�?            logger.info(f"执行命令: {' '.join(cmd)}")
            print(f"\n正在提取API信息: {repo_name}")
            ...
            while True:
                ...
                if output == '' and process.poll() is not None:
                    break
                if output:
                    # 清理输出并显示进度信�?                    line = output.strip()
                    if line and not line.startswith('DEBUG'):
                        print(f"  [文件] {line}")
                        # 强制刷新输出缓冲�?                        sys.stdout.flush()
            
            result = subprocess.CompletedProcess(
                args=cmd,
                returncode=process.returncode,
                stdout="",  # 已经实时显示�?                stderr=""
            )
            
            # 记录详细的执行结�?            logger.info(f"命令执行完成，返回码: {result.returncode}")
            if result.stdout:
                    ...
                    try:
                        with open(output_file, 'r', encoding='utf-8') as f:
                            ...
                            if total_functions == 0:
                                logger.warning(f"extract完成但未找到任何函数: {repo_name} (可能是非支持语言的库或空�?")
                                return "NO_FUNCTIONS_FOUND"  # 特殊标记表示没有找到函数
                            else:
                                logger.info(f"extract完成: {output_file} (找到 {total_functions} 个函�?")
                                return output_file
                    except Exception as e:
                        logger.error(f"无法解析extract输出文件: {output_file}, 错误: {str(e)}")
                        return None
                else:
                    logger.error(f"extract输出文件不存�? {output_file}")
                    return None
            else:
                # 检查是否是KeyError相关的错�?                if "KeyError" in result.stderr and "direct_call_weight" in result.stderr:
                    logger.warning(f"extract遇到KeyError，但可能已生成部分结�? {repo_name}")
                    # 检查是否仍然生成了输出文件
                    output_file = os.path.join("output/extract_output", f"{repo_name}_api_extraction.json")
                    if os.path.exists(output_file):
                        logger.info(f"尽管有KeyError，但找到了输出文�? {output_file}")
                        return output_file
                    else:
    ...
    def run_description(self, extraction_file: str) -> Optional[str]:
        ...
        try:
            # 构建命令，添�?-skip-complexity参数以使用默认moderate复杂�?重要�?            cmd = [
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(complete_file):
                    ...
                    logger.error(f"description输出文件不存�? {complete_file}")
                    return None
            else:
...
# 创建管理�?manager = CodeEmbeddingManager(config)
...
# 保存向量化器到vectorizers子文件夹（如果是TF-IDF�?if config.model_type == 'tfidf':
...
"""
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(index_file):
                    ...
                    logger.error(f"vector_embedding输出文件不存�? {index_file}")
                    return False
            else:
    ...
    def run_ingest(self, complete_file: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # 创建标记文件表示ingest已完�?                os.makedirs("output/ingest_output", exist_ok=True)
    ...
    def process_single_repo(self, repo_url: str, progress_tracker: ProgressTracker = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"开始处理仓�? {repo_name} ({repo_url})")
            ...
            if extraction_file:
                ...
                # 检查是否是没有找到函数的情�?                if extraction_file == "NO_FUNCTIONS_FOUND":
                    logger.info(f"跳过后续步骤: {repo_name} (未找到任何函�?")
                    result['step_details']['extract'] = {
                        'function_count': 0,
                        'file_count': 0,
                        'skip_reason': 'No functions found (possibly non-supported language repository or empty repository)'
                    }
                    result['steps']['description'] = True  # 标记为跳�?                    result['steps']['vector_embedding'] = True  # 标记为跳�?                    result['steps']['ingest'] = True  # 标记为跳�?                    result['step_details']['description'] = {'skip_reason': 'No functions to process'}
                    ...
                    if progress_tracker:
                        ...
                        progress_tracker.end_stage("函数描述生成 (Description)", "跳过 (无函�?")
                        progress_tracker.end_stage("向量嵌入 (Vector Embedding)", "跳过 (无函�?")
                        progress_tracker.end_stage("Neo4j导入 (Ingest)", "跳过 (无函�?")
                ...
                # 获取extract结果的统计信�?                try:
                    with open(extraction_file, 'r', encoding='utf-8') as f:
                        ...
                        extract_details = f"提取�?{function_count} 个函数，{extract_data.get('total_files', 0)} 个文�?
                except:
            ...
            if complete_file:
                ...
                # 获取description结果的统计信�?                try:
                    with open(complete_file, 'r', encoding='utf-8') as f:
                        ...
                        desc_details = f"生成�?{desc_count} 个函数的描述"
                except:
                    desc_details = "描述生成完成"
    ...
    def process_all_repos(self):
        ...
        logger.info(f"开始批量处�?{len(repo_urls)} 个仓�?)
        ...
        # 显示最终结�?        self.print_final_summary()
    ...
    def print_final_summary(self):
        ...
        logger.info(f"成功�? {(self.stats['successful_repos'] / self.stats['total_repos'] * 100):.1f}%")
        ...
        # 显示失败的仓�?        failed_repos = [r for r in self.stats['repo_results'] if not r['success']]
        if failed_repos:
            logger.info("\n失败的仓�?")
...
def main():
    """主函�?""
    parser = argparse.ArgumentParser(description='批量处理多个GitHub�?)
    ...
    parser.add_argument('--neo4j-user', default='neo4j', help='Neo4j用户�?)
    parser.add_argument('--neo4j-password', default='90879449Drq', help='Neo4j密码')
    parser.add_argument('--neo4j-database', help='Neo4j数据库名')
    parser.add_argument('--clean-db', action='store_true', help='清空数据�?)
    parser.add_argument('--filter-large-functions', action='store_true', help='过滤大函�?)
    parser.add_argument('--concurrent', type=int, default=4, help='并发�?)
    parser.add_argument('--batch-size', type=int, default=32, help='批处理大�?)
